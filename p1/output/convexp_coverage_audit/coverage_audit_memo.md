...
1. **Fix the corrupt `p1/t2_wrds/waves.csv`** (double-schema; regenerate 4-col cleanly).
   *(Regenerated during this audit; the "verify `build_waves.py` doesn't re-corrupt it"
   ...
   and the dead 7-column `main()` was the one that actually ran, so the next invocation
   would have overwritten the clean file and skipped `waves_members.csv` entirely. Same
   defect class as item 0. Scaffold removed, file regenerated by the pipeline itself,
   invariants pinned in `p1/tests/test_build_waves.py`.)*
