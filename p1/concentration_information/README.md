...
- **下一轮准备、尚未执行：[固定 earnings 公司信息检验 prompt](20260922_earnings_information_test/NEXT_EXECUTION_PROMPT.md)**。程序从既有23股选择六个发行人，最多48次2023/2024公告及固定对照；在真实公告会话中比较路径、同目标条件增量和后续调整。Sol/medium主执行、Terra/medium数据工程、Sol/high末端独立复核。先综合已有普通日/FOMC证据，再运行一次有限跨年检验；时段差异不当消息类型效应，宏观/独有信息区分不自动称新贡献。本轮仅准备prompt，没有启动SCC、购数或代理。
