...
- **待 PI 采用的下一轮 prompt：[固定 XOM/SPY 可观测路径](20260921_spy_xom_event_packet/NEXT_EXECUTION_PROMPT.md)**：按用户要求加快形成实际结果，提出有限规格修订：必做固定单事件两证券路径，条件具备时补滞后持仓代理，历史篮子追索限时一次；I/B/E/S 惊喜信号不进入本轮。此文件尚未执行，须用户随后要求执行才采用新增诊断值访问范围；不能据此声称价格发现或主假设已通过。
