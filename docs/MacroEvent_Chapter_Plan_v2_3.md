...
## Standby dissertation chapter — complete research plan (v2.3, 2026-08-19)
...
>
> **v2.3 is a mechanism/econometric hardening pass. The paper is NOT broadened.** It fixes a
...
**5.1.1 Required series (R1a must confirm each exists in USMPD, or name the substitute).**

| Slot | What it must be | Status |
|---|---|---|
| Policy instrument | the high-frequency change in the registered policy-sensitive rate instrument (contract, maturity and window all fixed by R1a from the official documentation, quoted) | `NEED_INFO` — R1a |
| Equity series | the high-frequency change in a broad equity index **in the same window** | `NEED_INFO` — R1a must confirm USMPD carries one; if it does not, the equity leg comes from a named external source and that becomes a data dependency |
| Window | the exact window (minutes before/after) the official documentation defines | `NEED_INFO` — R1a, quoted verbatim |
...
### 5.2 FOMC event definition — frozen (new in v2.3)

USMPD distinguishes statement windows, press-conference windows, combined monetary events,
and minutes releases. Leaving this open would let **delayed press-conference news masquerade
as slow price adjustment** — which is fatal to spine 2, whose whole content is an adjustment
half-life.

**Registered:**

1. **Primary event = the STATEMENT window.** One clean, instantaneous release.
2. **The press conference is a SEPARATE registered event** with its own shock, entering the
   same specification simultaneously. It is never pooled into the statement shock in the
   primary.
3. **The combined monetary event** is reported as a robustness aggregation only.
4. **Minutes releases are excluded** from the primary and reported separately if at all.
5. **Half-life is measured from the statement timestamp with the press-conference window
   explicitly controlled.** Any adjustment attributed to slow propagation must survive that
   control, or it is reported as press-conference news.

R1a must confirm USMPD's window taxonomy and timestamps; if the database does not separate
statement from press conference, item 2 becomes a data dependency and spine 2's half-life
claim is qualified until it is met.
...
**6.1.1 ONE primary outcome, chosen before seeing results (v2.3 — replaces the 3-of-5 vote).**
v2.2's "predicts a majority of five proxies" rule had no economic foundation; a vote across
heterogeneous proxies is not a test. Registered instead:

> **PRIMARY: the constituent's price sensitivity to the ETF's creation/redemption activity.**
> Does a stock with higher predetermined L_tilt^pre respond more to the ETF's arbitrage flow?

That outcome is chosen because it is the one that *is* the mechanism: the arbitrage channel
operating on the stock. Basket weight is rejected as primary precisely because it is close to
mechanical in holdings — it would be passed by construction rather than by economics.

**Secondary corroboration** (reported, never decisive, never aggregated into a score):
basket inclusion/weight · premium/discount convergence speed · ETF–constituent lead-lag
strength · constituent order imbalance around ETF flows.

**6.1.2 If the primary fails (v2.3 — the "redefine from what worked" rule is DELETED).**
...
**6.1.3 The prereg carve-out this requires — narrow, registered, machine-enforced.** Because
...
- it touches **no outcome variable of the study** — no propagation outcome, no return, no
  spine 1–5 quantity;
...
### 8.1 Inference hierarchy and effective clusters (new in v2.3)

**The rule: power comes from independent events and independent treatments, never from
intraday row counts.** An intraday panel multiplies rows without adding a single independent
FOMC event or conversion wave, and any standard error that improves because the sampling
interval got finer is an artefact.

**What the design actually has** (computed 2026-08-19 from the committed event set; the FOMC
count needs R1a's calendar and is `NEED_INFO`):

| Dimension | Count | Note |
|---|---|---|
| Conversion waves in frame | **53** | after the 2026-08-19 sample decision (waves_end 2025-06-30) |
| Converting funds in frame | **83** | |
| Trust-level families | **46** | **overstates independence** — e.g. "Fidelity Summer Street Trust" and "Fidelity Commonwealth Trust II" are one economic sponsor |
| Adviser-level sponsors | `NEED_INFO` | must be built by mapping trusts to advisers; this is the number that matters for sponsor clustering |
| Largest family share of funds | 8.2% | reassuring against single-sponsor dominance, subject to the adviser roll-up |
| Independent FOMC events | `NEED_INFO` | R1a's calendar; ~8 scheduled meetings/year over the announcement window, **before** the statement/press-conference split of §5.2 |
| Effective treatment clusters after intraday coverage filters | `NEED_INFO` | **must be recomputed after G7**, since coverage failures drop waves, not just rows |

**Registered inference ladder.**

1. Two-way clustering on **announcement date × wave** remains the primary.
2. **Sponsor (adviser-level) clustering** is reported alongside, because waves within one
   adviser are not independent draws.
3. With clusters this few, **wild cluster bootstrap** (or another justified small-cluster
   procedure) is the primary inference for the headline coefficients, not a robustness row.
4. **Randomization inference** stays in the main table, as in v2.1.
5. The **effective cluster count is reported in every table note**, post-coverage-filter.
6. If the effective count falls below the registered warning level, the paper says so in the
   text rather than in an appendix.
...
| **2–3** | **G7 — INTRADAY COVERAGE (BLOCKING).** Pilot: 30 treated + 30 control × 20 announcement days. **Audited on BOTH sides (v2.3):** the converted ETF *and* its constituents, at the §5.2 statement and press-conference timestamps. Small-cap constituents are the stress test; ETF-side coverage alone proves nothing. Vendor substitutes run their §4.1 agreement validation here | **≥70% usable coverage including small caps, on both sides, at the required timestamps.** FAIL → stock-level architecture claims cannot be made; route to §10 **exit F** |
| **2–3** | **G8 — FIRST-STAGE MECHANISM VALIDATION (HIGHEST PRIORITY; timing corrected in v2.3).** §6.1: predetermined L_tilt^pre → **post-conversion** arbitrage connectivity, ONE registered primary outcome | Primary outcome significant with the implied sign → **licensed**. Otherwise → **retired** from the headline; a redesigned measure re-enters only via §6.1.2's external-sample or cross-fitting route; if nothing survives → §10 **exit E**. **Requires the §6.1.3 prereg carve-out to be signed before it runs** |
| **1–2** | **G9 — PORTFOLIO CONTINUITY (new in v2.3, BLOCKING).** The design's central premise is that only the wrapper changed. Verify it rather than assert it: manager/adviser and stated objective continuity across the switch, plus **pre/post holdings overlap, portfolio-weight correlation, and turnover, computed per wave** | Registered thresholds `[VERIFY-IN-GATE-0]`. Waves failing continuity are **flagged and reported separately**; if continuity is weak across the sample, the "delegation-constant" language is withdrawn and the design is qualified as wrapper-plus-portfolio change. Cheap, uses holdings data already required, and runs first |
