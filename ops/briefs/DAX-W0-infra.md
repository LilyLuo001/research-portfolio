# PRIME-BLOCK BRIEF — DAX-W0-infra
_generated 2026-07-09T14:43:17.932294Z — a fresh session needs zero
...
2. claim the task:  `python ops/runner/lease.py claim DAX-W0-infra --account A`
   (if this errors non-fast-forward, another seat beat you — pick the next brief)
3. `git switch -c task/DAX-W0-infra`  (or reuse the worktree for this task)

## the task
repo guard: analysis/outcomes/ refuses to run until memo tag v1.0-preregistered exists; CI NDA grep.

Paste the corresponding verbatim prompt from the manual/amendment for DAX-W0-infra here,
...
- merge task/DAX-W0-infra → main; `python ops/runner/runner.py --complete DAX-W0-infra`
