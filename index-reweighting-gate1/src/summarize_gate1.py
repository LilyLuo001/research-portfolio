...
def build_summary(root: Path) -> dict[str, object]:
    ...
    binding_by_event = {
        row.get("event_id", ""): row.get("binding_status", "") for row in events
    }
    confirmed_binding_groups = {
        row.get("event_group_id", "")
        for row in events
        if row.get("binding_status") == "confirmed_binding"
        and row.get("event_group_id", "")
    }
    confirmed_other_intervention_groups = {
        row.get("event_group_id", "")
        for row in events
        if row.get("binding_status") == "confirmed_other_intervention"
        and row.get("event_group_id", "")
    }

    meeting_scope = {
        row.get("meeting_id", ""): row.get("calendar_scope", "") for row in fomc
    }
    ...
    primary_unique_band_pairs: dict[str, set[tuple[str, str]]] = {
        "20": set(),
        "40": set(),
        "60": set(),
    }
    binding_primary_band_pairs: dict[str, set[tuple[str, str]]] = {
        "20": set(),
        "40": set(),
        "60": set(),
    }
    for row in support:
        ...
        for band in ("20", "40", "60"):
            if yes(row.get(f"within_{band}td", "")):
                ...
                if meeting_scope.get(meeting, "").lower() == "primary_2018_2025":
                    primary_unique_band_pairs[band].add((group, meeting))
                    if binding_by_event.get(row.get("event_id", "")) == "confirmed_binding":
                        binding_primary_band_pairs[band].add((group, meeting))
    ...
    return {
        ...
        "institutional": {
            ...
            "rule_source_grade_counts": counts(rules, "rule_source_grade"),
            "event_rows": len(events),
            "unique_event_ids": len(real_event_ids),
            "unique_event_groups": len(real_event_groups),
            "confirmed_binding_unique_event_groups": len(confirmed_binding_groups),
            "confirmed_other_intervention_unique_event_groups": len(
                confirmed_other_intervention_groups
            ),
            "event_binding_status_counts": counts(events, "binding_status"),
            "event_classification_evidence_grade_counts": counts(
                events, "classification_evidence_grade"
            ),
            "event_assignment_evidence_grade_counts": counts(
                events, "assignment_evidence_grade"
            ),
        },
        "assignment": {
            "dose_rows": len(doses),
        ...
        "common_news": {
            ...
            "eligible_primary_2018_2025_meeting_ids": len(
                {
                    row.get("meeting_id", "")
                    for row in fomc
            ...
            "eligible_all_scope_meeting_ids": len(
                {
                    row.get("meeting_id", "")
                    for row in fomc
                    if yes(row.get("eligible_common_news", ""))
            ...
            "support_role_counts": counts(support, "support_role"),
            "anticipation_window_status_counts": counts(
                support, "anticipation_window_status"
            ),
            ...
            "primary_2018_2025_event_group_meeting_pairs_by_band": {
                band: len(pairs)
                for band, pairs in primary_unique_band_pairs.items()
            },
            "confirmed_binding_primary_event_group_meeting_pairs_by_band": {
                band: len(pairs)
                for band, pairs in binding_primary_band_pairs.items()
            },
            "confirmed_binding_primary_distinct_meetings_by_band": {
                band: len({meeting for _, meeting in pairs})
                for band, pairs in binding_primary_band_pairs.items()
            },
        },
        "design": {
