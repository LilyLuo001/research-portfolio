...
STAGE_A_GAP_BLOCKERS = {
    "exact_assignment_inputs": ("yes", "yes"),
    "target_membership": ("yes", "yes"),
    "holdings_timing_metadata": ("no", "yes"),
    "holdings_snapshot_gaps": ("no", "no"),
    "provider_issuer_float_mapping": ("yes", "yes"),
}
...
def build_comparison_rows(
    ...
    for event in events:
        ...
        assignment_grade = strongest_grade(
            grades or [event.get("assignment_evidence_grade", "U")]
        )
        binding_status = event.get("binding_status", "")

        if binding_status == "confirmed_other_intervention":
            status = "OTHER_INTERVENTION_CONTEXT_NOT_CANDIDATE_TREATMENT"
            matrix_status = "NOT_APPLICABLE_INTERFERENCE_CONTEXT"
        elif binding_status == "confirmed_nonbinding":
            status = "CONFIRMED_NONBINDING_CONTEXT"
            matrix_status = "NOT_APPLICABLE_NO_CONFIRMED_CAP_TREATMENT"
        elif binding_status == "outside_applicable_regime":
            status = "OUTSIDE_APPLICABLE_REGIME"
            matrix_status = "NOT_APPLICABLE_OUTSIDE_REGIME"
        elif complete and signed_values:
        ...
        output.append(
            {
                ...
                "treatment_object": (
                    "other documented index change retained as interference context"
                    if binding_status == "confirmed_other_intervention"
                    else "rule-assigned index-weight change or event-level intervention; not ETF holdings"
                ),
...
def build_gap_rows(
    ...
    # Generic industry/index-membership tables are not proof that the two target
    # index families are covered.  The Stage A target-search row is the only
    # resolving contract for this gap.
    has_membership = any(
        row.get("evidence_key") == "LOCAL:target_index_membership_search"
        and row.get("status") == "TARGET_IDENTIFIED_WITH_CONSTITUENT_ROWS"
    ...
    for source in stage_a_gaps:
        ...
        if gap_id not in STAGE_A_GAP_BLOCKERS:
            raise ValueError(
                f"Stage A gap {gap_id!r} has no explicit blocker adjudication"
            )
        blocks_assignment, blocks_measurement = STAGE_A_GAP_BLOCKERS[gap_id]
        gaps.append(
            {
                ...
                "blocks_gate1_assignment": blocks_assignment,
                "blocks_future_measurement": blocks_measurement,
                "source_locator": source.get("source_locator", "remaining_input_gaps_stage_a.csv"),
                "next_action": source.get("next_action", ""),
                "notes": "Imported from the executed Stage A archive audit; blocker roles were explicitly adjudicated in the integration builder.",
            }
        )
