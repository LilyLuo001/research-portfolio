...
A final, tightly predeclared exploratory model places two continuous summaries in the stock equation together. The family-balanced consensus component has a negative coefficient, -0.0404, while the AIOE-versus-Eloundou family-disagreement component has a positive coefficient, 0.0309; both fixed wild-score intervals exclude zero. The result reinforces the paper's central distinction. A broad consensus dimension can carry the aggregate negative association while the choice between families remains consequential. The disagreement component is not every architecture-specific difference, and neither coefficient identifies a causal AI effect.
...
The joint result is accepted as estimated: the negative consensus association coexists with an oppositely signed incremental between-family dimension. Because larger `G` means greater AIOE relative to Eloundou exposure, the result says that, holding consensus fixed, occupations tilted toward AIOE have a less negative young-relative post association. It does not reveal which family is correct, and `G` is not all architecture-specific disagreement.
...
The appendix contains the full architecture and correlation matrices; exposure lineage and occupational bridges; residual-treatment and conditional-information support; all native and common-support stock models; the Q2–Q5 profile; paired-MDE reconciliation; event-study coefficients; computerization and remotability results; two-way dependence sensitivity; adjacent-link construction; separate exit, outflow, and entry-destination estimands; all 15 κ results; hard-benchmark taxonomy and implementation; the partly mechanical consensus-distance organization; the one joint F+G plan and result; and complete reproducibility receipts.
