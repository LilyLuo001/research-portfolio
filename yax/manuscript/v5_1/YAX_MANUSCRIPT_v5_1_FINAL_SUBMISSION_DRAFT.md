...
A fixed-treatment delete-one-occupation audit classifies this coefficient as moderately influenced but not fragile (`LOCO-B2`). Across all 468 deletions, the estimate remains negative, ranging from -0.1424 to -0.1106; the largest absolute movement is 0.0205 (15.7% of the full estimate) after deleting fast food and counter workers. No occupation was removed from the headline as a result of this diagnostic.
...
The paired beta-minus-alpha contrast is -0.0324 log points (paired SE 0.0370; 95% CI [-0.1023, 0.0376]; `p=.403`). The design does not detect a difference and does not establish equivalence.

The prospective simulations materially overstated realized precision. For the primary beta-by-Webb design, the prospective synthetic SE was 0.01217 and the realized SE is 0.04441, a ratio of 3.649. For the paired beta-minus-alpha contrast, the corresponding values are 0.01167 and 0.03697, a ratio of 3.167. A code audit rejects the specific claim that the simulations treated occupation-month residual shocks as independent across months: each draw retained an ordered historical residual path and applied one sign across the whole path for each occupation. The simulations did not impose a parametric serial-correlation process and differ from the realized setting in other ways, so the record does not isolate a unique cause of the calibration gap. The old MDEs remain design-history provenance, not evidence that the realized analysis was well powered, and this disclosure does not change the realized intervals or p-values.

### 6.2 Consensus and between-family evidence

The already-estimated two-dimensional F/G model can be expressed equivalently in the original family-centroid basis. Per weighted SD, the implied AIOE-family coefficient is +0.0249 (covariance-transformed normal 95% interval [-0.0149, 0.0648]), whereas the implied Eloundou-family coefficient is -0.0615 (normal interval [-0.1084, -0.0146]). The original-unit AIOE-minus-Eloundou contrast is +0.09590. Because that contrast is exactly `b_G/s_G`, it retains the existing G wild-score interval [0.00559, 0.18621] and `p=.040`. This is a reparameterization of the same exploratory model, not independent corroborating evidence. The individual A and E intervals combine F and G and use the serialized common-draw covariance with a normal 1.96 critical value; they are not wild-score intervals because draw-level F/G shifts were not retained.

The exact map begins from `F=(A+E)/2` and `G=(A-E)/2`. Because the executed model standardized F and G separately, the coefficients cannot be converted by simply adding and subtracting them. On the frozen support, the weighted SDs are `s_A=0.99865`, `s_E=0.86665`, `s_F=0.87774`, and `s_G=0.32214`. Applying the scale-correct transformation gives +0.02496 per original AIOE-centroid unit and -0.07094 per original Eloundou-centroid unit and reproduces the fitted F/G contribution to maximum absolute error `2.78e-17`. No A+E model was estimated.

In the exploratory continuous joint specification, the negative conditional stock association therefore loads more heavily on the Eloundou-family centroid than on the AIOE-family centroid. An outcome-free construction audit is only `G-PARTIAL`: removing beta or broad leaves G highly similar, but removing alpha lowers the weighted level/rank correlations to 0.865/0.855, retains 72.5% of frozen-Q1 and 69.5% of frozen-Q5 weight, and changes zero-direction for 17.9% of employment. No leave-one-measure-out labor-outcome models were run. The fixed-treatment occupation-influence audit is `LOCO-G1`: all 444 leave-one-occupation-out G estimates remain positive, from 0.02513 to 0.03599, with a maximum absolute movement of 0.00577; deleting driver/sales workers and truck drivers produces the largest change. This occupation stability does not erase construction sensitivity. The result partly reflects alpha's distinctive position and remains bounded evidence about the frozen construction, not a general family ranking. It does not reveal which family is correct, imply that AIOE has no effect, or make `G` a measure of all architecture-specific disagreement.

For completeness, the one authorized joint continuous model includes standardized `F`, standardized `G`, and Webb together. The consensus coefficient is -0.04036 per SD (occupation-cluster SE 0.01407; wild-score CI [-0.06925, -0.01147]; `p=.007`). Conditional on `F` and Webb, the between-family coefficient is +0.03089 (SE 0.01486; wild-score CI [0.00180, 0.05999]; `p=.040`). The centered common-draw covariance is -0.00009825 (correlation -0.464), and the max-|t| joint test of both coefficients equal to zero gives `p=.008`. Separately, an exploratory F-quintile model on the same 444 occupations produces -0.1285 log points (95% wild-score CI [-0.2185, -0.0386]). This shows that a transparent equal-family-weight summary can reproduce the negative aggregate tail direction; it does not establish a uniquely correct exposure variable.
