#!/usr/bin/env python3
"""Build, submit, poll, and privately download the corrected R3 link-weight patch.

The API credential is read from ``IPUMS_API_KEY`` only. No credential or
restricted download URL is written to the repository receipt.
"""
...
COLLECTION = "cps"
VERSION = 2
REPLACED_MARCH_YEARS = tuple(range(2017, 2022))
VARIABLES = (
...
def build(base_path: pathlib.Path, output_path: pathlib.Path) -> dict:
    ...
    source_samples = list(base["samples"])
    if len(source_samples) != 114 or "cps2025_10s" in source_samples:
        raise RuntimeError("historical sample set differs from the authenticated 114-sample calendar")
    replacement = {f"cps{year}_03s": f"cps{year}_03b" for year in REPLACED_MARCH_YEARS}
    corrected = [replacement.get(sample, sample) for sample in source_samples]
    if len(set(corrected)) != 114:
        raise RuntimeError("corrected sample set is not unique")
    for year in REPLACED_MARCH_YEARS:
        if f"cps{year}_03b" not in corrected or f"cps{year}_03s" in corrected:
            raise RuntimeError(f"March {year} replacement failed")
    ages = [str(value) for value in range(16, 76)]
    spec = {
        "description": (
            "YAX R3 corrected-calendar link-weight patch: 114 BMS samples, "
            "2017-2021 March Basic replacement, ages 16-75, adjacent-month and "
            "adjacent-year longitudinal weights."
        ),
        "dataStructure": {"rectangular": {"on": "P"}},
        "dataFormat": "csv",
        "caseSelectWho": "individuals",
        "samples": {sample: {} for sample in corrected},
        "variables": {
            variable: ({"caseSelections": {"general": ages}} if variable == "AGE" else {})
...
def sanitized(response: dict, spec_path: pathlib.Path) -> dict:
    ...
    return {
        "record": "YAX R3 corrected-calendar longitudinal-weight request",
        "collection": COLLECTION,
        "api_version": VERSION,
        "extract_number": response.get("number"),
        "extract_status": response.get("status"),
        "checked_at_utc": datetime.now(timezone.utc).isoformat(),
        "spec_repo_path": "yax/revision/substantive_r3_20260905/flows/CORRECTED_WEIGHT_PATCH_SPEC.json",
...
def submit(spec_path: pathlib.Path, receipt_path: pathlib.Path) -> dict:
    response = api("POST", endpoint(), json=json.loads(spec_path.read_text(encoding="utf-8")))
...
def refresh(receipt_path: pathlib.Path, spec_path: pathlib.Path) -> tuple[dict, dict]:
...
def download(receipt_path: pathlib.Path, spec_path: pathlib.Path, private_dir: pathlib.Path) -> dict:
    receipt, response = refresh(receipt_path, spec_path)
    ...
    for name, metadata in (response.get("downloadLinks") or {}).items():
        if name not in {"data", "ddiCodebook", "basicCodebook"}:
            continue
        destination = private_dir / pathlib.Path(metadata["url"]).name
        partial = destination.with_suffix(destination.suffix + ".part")
        with requests.get(metadata["url"], headers={"Authorization": key()}, timeout=120, stream=True) as remote:
        ...
        manifest["files"][name] = {
            "path": str(destination), "bytes": destination.stat().st_size, "sha256": actual,
        }
...
def main() -> int:
    ...
    status_parser.add_argument("--spec", type=pathlib.Path, required=True)
    status_parser.add_argument("--receipt", type=pathlib.Path, required=True)
    download_parser = sub.add_parser("download")
    download_parser.add_argument("--spec", type=pathlib.Path, required=True)
    ...
    if args.command == "build":
        spec = build(args.base, args.output)
        output = {"status": "BUILT", "samples": len(spec["samples"]), "variables": list(spec["variables"])}
    elif args.command == "submit":
        output = submit(args.spec, args.receipt)
    elif args.command == "status":
        output, _ = refresh(args.receipt, args.spec)
    else:
        output = download(args.receipt, args.spec, args.private_dir)
