...
def main() -> None:
    ...
    check(ss.successful_full_refits.min() >= int(np.floor(.90 * args.simulation_draws)), "at least 90 percent of simulation refits converge up to integer rounding")
    check(len(simulation) >= int(np.floor(.90 * 3 * 2 * args.simulation_draws)), "simulation success rows clear threshold")
    ...
    check(all(row.get("workstream") == "INF-05" for row in failures), "any retained failures are simulation-only")
