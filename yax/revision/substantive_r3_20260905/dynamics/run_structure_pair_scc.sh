...
#$ -j y
#$ -N yax_r3_structpair
#$ -o /project/econdept/qluo/yax-substantive-revision-20260905/agents/dynamics/structure_pair_execution.log
set -euo pipefail
...
"$PYTHON" "$AGENT/run_structure_pair.py" \
  ...
  --rebuilt-membership "$REBUILT" \
  --output-dir "$OUT"

"$PYTHON" "$AGENT/selfcheck.py" --output-dir "$OUT" --require-structure-pair
