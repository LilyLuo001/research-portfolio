...
Written before any fix; **items 1–6 were then applied the same day** — see the fix
log at the end for what landed. Each finding below keeps its original diagnosis
and reproduction so the fix can be checked against what it was meant to solve.
Item 7 remains open: it changes what `overall_pass` certifies, and this is a
preregistration-sensitive pipeline, so it waits for owner sign-off._
...
|---|---|---|
| ✅ Fixed | R-1 A10 crashes on a contract-conformant P1 file | Adapter + 9 boundary tests |
| ✅ Fixed | R-2 blank `permno`; wrong input list | Refusal guard + CLAUDE.md corrected |
| 🟠 **OPEN** | R-3 A11 and A14 are vacuous in the CLI path | **Needs sign-off (item 7)** |
| ✅ Fixed | R-4 documented `betas`/`convexp` schemas were wrong | Docstring corrected |
| ✅ Fixed | R-5 fixtures encoded assumptions, not contracts | `tests/test_p1_boundary.py` |
| ✅ Fixed | R-6 A6 scan missed most of `refraction/` | Widened; `tests/` excluded |
| ✅ Fixed | R-7 A9 divided by `1 − w_i` unguarded | Degenerate rows reported, not divided |
...
Items 1–6 applied. Refraction suite: **51 passed** (42 before + 9 boundary tests).

| # | Fix | Landed |
|---|---|---|
| 1 | `_read_p1_convexp()` adapter — accepts `wave_id`, normalises to `wave`; raises on a frame carrying neither | ✅ |
| 2 | `tests/test_p1_boundary.py` — 9 tests building convexp to P1's **frozen contract**, incl. a check that the contract still keys on `wave_id` | ✅ |
| 3 | Corrected the documented `betas` (adds `beta_b_loo`) and `convexp` schemas | ✅ |
| 4 | A6 scans all of `refraction/`, excluding `tests/` | ✅ |
| 5 | A9 reports single-name baskets instead of dividing by zero | ✅ |
| 6 | `assert_p1_join_key_usable()` + corrected input table in `refraction/CLAUDE.md` | ✅ |
| 7 | `strict=True` fail-closed for A11/A14 | ❌ **open — needs sign-off** |

**Worth recording: widening the A6 scan (fix 4) immediately fired.** The hit was
`tests/test_assert_panel.py:49`, the tampered-world fixture that *writes* a magic
`w_shrink` literal in order to prove A6 catches one. Scanning the suite makes the
check fail on its own bait, so `tests/` is now excluded by name with that reason
in the docstring. The narrow scan had been hiding a scope bug behind a false
negative; the wide scan found a false positive first.

**Still open — item 7.** A11 compares the panel against itself and A14 returns
pass when no upstream frames are supplied; `main()` supplies neither, and both sit
in `HARD`. Until this is fixed, a manifest generated by the CLI can truthfully
report `A11: pass, A14: pass` when neither ran. The manual makes A14 an R2
acceptance criterion (§167), so the fix is almost certainly what the manual
intends — but it changes what `overall_pass` certifies, which belongs in
`ops/decisions.md` rather than in a quiet commit.
